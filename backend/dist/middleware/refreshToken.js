...
export const refreshToken = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userModel = yield userModel.findById(decoded.id);
        if (userModel === null) {
            res.status(404).json({ success: false, error: 'userModel not found' });
            return;
        }
        // No roles implemented yet
        const token = jwt.sign({ id: userModel._id }, privateKey, { expiresIn: '1h' });
        const newRefreshToken = jwt.sign({ id: userModel._id }, privateRefreshKey, { expiresIn: '7d' });
        yield Token.create({ userId: userModel._id, token: newRefreshToken });
